...
indexofnorthboundRoutes = [i for i in totalPatterns if patternResponse['bustime-response']['ptr'][i]['rtdir'] == 'Northbound']
totalRoutes = range(len(indexofnorthboundRoutes))
patternFind = lambda k: patternResponse['bustime-response']['ptr'][indexofnorthboundRoutes[k]]
...
getsequencePatternLat = [[patternFind(k)['pt'][j]['lat'] for j in range(len(patternFind(k)['pt']))] for k in totalRoutes]
# Using the List Comprehension syntax to shorten the code, create a list for each route a list of longitudes
getsequencePatternLon = [[patternFind(k)['pt'][j]['lon'] for j in range(len(patternFind(k)['pt']))] for k in totalRoutes]
# Join the Lat and Lon for each PIDs separated by ',' and put each in a list and list each per PID
joinedLatLon = [[','.join([str(getsequencePatternLat[k][j]),str(getsequencePatternLon[k][j])]) for j in range(len(patternFind(k)['pt']))] for k in totalRoutes]
...
joinedstopLatLon = [','.join([str(stopsResponse['bustime-response']['stops'][k]['lat']),str(stopsResponse['bustime-response']['stops'][k]['lon'])]) for k in range(len(stopsResponse['bustime-response']['stops']))]
